...
export default function CardList({ deckId, cards }: { deckId: string; cards: Card[] }) {
  ...
  return (
    <div className="flex flex-col gap-2">
      {cards.map((card) =>
        editingId === card.id ? (
          <div key={card.id} className="rounded-md border border-black/10 bg-card p-2 dark:border-white/10">
            <CardForm
              initial={card}
              submitLabel="Save"
          ...
          <div
            key={card.id}
            className="grid grid-cols-1 items-center gap-1 rounded-md border border-black/10 bg-card p-3 sm:grid-cols-[1fr_1fr_2fr_auto] sm:gap-2 dark:border-white/10"
          >
      ...
      {adding ? (
        <div className="rounded-md border border-black/10 bg-card p-2 dark:border-white/10">
          <CardForm
